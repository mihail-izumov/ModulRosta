    ...
    <div class="mr-history-container">
      ...
      <div class="mr-history-filters">
        ...
        <button :class="['mr-identity-btn', { active: identityMode }]" @click="toggleIdentityMode">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 7V5a2 2 0 0 1 2-2h2"/><path d="M17 3h2a2 2 0 0 1 2 2v2"/><path d="M21 17v2a2 2 0 0 1-2 2h-2"/><path d="M7 21H5a2 2 0 0 1-2-2v-2"/><circle cx="12" cy="12" r="1"/></svg>
          Айдентика МР
        </button>
      </div>
          ...
          <div v-if="index < 5" :class="['mr-history-row', { hidden: !isVisible(project) }]">
            ...
            <div class="mr-history-build">
              <div :class="['mr-build-block', getStatusClass(project.status)]" @click.stop="openDetails(project.id)">
                ...
                <span class="mr-build-label">ДЕТАЛИ</span>
              </div>
            </div>
          </div>
        </template>
        ...
        <div class="mr-modal-content mr-gallery-modal">
          ...
          <div class="mr-gallery-section">
            ...
            <div class="mr-gallery-slider">
              <div v-for="(img, idx) in currentProject?.images" :key="idx" :class="['mr-gallery-item', { expanded: expandedImage === idx }]" @click="toggleImageExpand(idx)"><span>{{ img }}</span></div>
            </div>
          </div>
        </div>
...
const expandedImage = ref<number | null>(null)
...
const allProjects = ref<Project[]>([
  ...
  { id: 'yurtrust', title: 'Айдентика и Веб', subtitle: 'ЮрТраст', website: '#', images: ['Главная', 'О компании', 'Услуги'], videos: [], tags: ['Айдентика', 'Веб'], caseUrl: null, behanceUrl: null, launchDate: '01.01.2024', buildTime: '28д', status: 'Запущен', mrBranded: true },
  ...
  { id: 'easybusy', title: 'Бизнес-сервис', subtitle: 'EASYBUSY', website: 'https://easybusy.ru', images: ['Главная', 'Сервисы', 'Тарифы', 'О компании', 'Кейсы', 'Контакты'], videos: [], tags: ['Айдентика', 'Web', 'Стратегия'], caseUrl: '#', behanceUrl: 'https://behance.net/orxaos', launchDate: '01.06.2020', buildTime: '28д', status: 'Запущен', mrBranded: true },
  { id: 'rawbites', title: 'Айдентика и Веб', subtitle: 'RAW BITES', website: '#', images: ['Главная', 'Продукция', 'О бренде'], videos: [], tags: ['Айдентика', 'Web', 'Стратегия'], caseUrl: null, behanceUrl: null, launchDate: '01.03.2020', buildTime: '28д', status: 'Запущен', mrBranded: true },
  ...
  { id: 'chishminsky', title: 'Айдентика и Веб', subtitle: 'Чишминский', website: '#', images: ['Главная', 'Продукция', 'О компании'], videos: [], tags: ['Айдентика', 'Веб', 'Стратегия'], caseUrl: null, behanceUrl: null, launchDate: '01.06.2018', buildTime: '35д', status: 'Запущен', mrBranded: true },
  { id: 'milimon', title: 'Производитель мебели', subtitle: 'Milimon', website: 'https://milimon.ru', images: ['Главная', 'Услуги', 'Контакты'], videos: [], tags: ['Стратегия', 'Веб'], caseUrl: null, behanceUrl: null, launchDate: '01.01.2016', buildTime: '21д', status: 'Запущен', mrBranded: false },
  { id: 'moroshka', title: 'Концепция и Айдентика', subtitle: 'Морошка', website: null, images: ['Концепт'], videos: [], tags: ['Концепция', 'Айдентика', 'Стратегия'], caseUrl: null, behanceUrl: null, launchDate: '01.01.2010', buildTime: '28д', status: 'Запущен', mrBranded: true }
])
...
function getUptime(p: Project): string {
  if (p.status === 'Скоро запуск') {
    ...
    if (m) {
      ...
      return `${d}д<span class="mr-blink">:</span>${h}ч<span class="mr-blink">:</span>${min}мин`
    }
    return 'T-30д'
  }
...
function openImages(id: string) { currentProjectId.value = id; expandedImage.value = null; imagesModalOpen.value = true; document.body.style.overflow = 'hidden' }
function closeImagesModal() { imagesModalOpen.value = false; expandedImage.value = null; document.body.style.overflow = '' }
function toggleImageExpand(idx: number) { expandedImage.value = expandedImage.value === idx ? null : idx }
...
<style scoped>
.mr-history-section { padding: 80px 24px; }
.mr-history-container { max-width: 1100px; margin: 0 auto; padding: 0 16px; }
...
.mr-identity-btn { display: flex; align-items: center; gap: 10px; padding: 12px 24px; background: linear-gradient(135deg, rgba(255, 85, 85, 0.2), rgba(255, 85, 85, 0.05)); border: 2px solid #ff5555; border-radius: 8px; color: #ff5555; font-family: 'JetBrains Mono', monospace; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; cursor: pointer; transition: all 0.3s ease; margin-left: auto; }
.mr-identity-btn:hover, .mr-identity-btn.active { background: #ff5555; color: white; box-shadow: 0 0 30px rgba(255, 85, 85, 0.5); }
...
.mr-history-row { display: grid; grid-template-columns: 100px 1fr auto 160px; align-items: center; padding: 16px 24px 16px 32px; border-bottom: 1px solid #222; transition: all 0.3s ease; gap: 24px; cursor: pointer; }
...
@media (max-width: 600px) {
  .mr-history-row { grid-template-columns: 56px 1fr auto; padding: 12px 16px; gap: 12px; }
  .mr-history-container { padding: 0 8px; }
}
...
.mr-history-subtitle { font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
/* Build block */
.mr-history-build { display: flex; align-items: center; width: 150px; flex-shrink: 0; }
.mr-build-block { display: flex; align-items: center; gap: 8px; padding: 8px 12px; background: transparent; border: 2px solid rgb(0, 255, 136); border-radius: 8px; cursor: pointer; transition: all 0.3s ease; position: relative; white-space: nowrap; }
...
.mr-build-label { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 700; letter-spacing: 2px; color: rgb(0, 255, 136); opacity: 0; transition: opacity 0.3s ease; }
.mr-build-block:hover svg, .mr-build-block:hover .mr-build-time { opacity: 0; }
.mr-build-block:hover .mr-build-label { opacity: 1; }
.mr-build-block:hover { background: rgba(0, 255, 136, 0.1); box-shadow: 0 0 20px rgba(0, 255, 136, 0.4); }
.mr-build-block.soon { border-color: #58a6ff; }
.mr-build-block.soon svg { color: #58a6ff; }
.mr-build-block.soon .mr-build-label { color: #58a6ff; }
.mr-build-block.soon:hover { background: rgba(88, 166, 255, 0.1); box-shadow: 0 0 20px rgba(88, 166, 255, 0.3); }
.mr-build-block.grounded { border-color: #7d8590; }
.mr-build-block.grounded svg { color: #7d8590; }
.mr-build-block.grounded .mr-build-label { color: #7d8590; }
.mr-build-block.grounded:hover { background: rgba(125, 133, 144, 0.1); box-shadow: 0 0 20px rgba(125, 133, 144, 0.3); }
...
.mr-accordion-toggle:hover { background: rgba(0, 255, 136, 0.02); color: rgb(0, 255, 136); }
...
.mr-modal-close { position: fixed; top: 30px; right: 30px; width: 70px; height: 70px; background: transparent; border: 2px solid #ff5555; color: #ff5555; cursor: pointer; border-radius: 14px; transition: all 0.3s ease; z-index: 10001; }
...
.mr-modal-close:hover { background: #ff5555; color: #000; }
.mr-modal-close.soon { border-color: #58a6ff; color: #58a6ff; }
.mr-modal-close.soon:hover { background: #58a6ff; }
.mr-modal-close.orbit { border-color: rgb(0, 255, 136); color: rgb(0, 255, 136); }
.mr-modal-close.orbit:hover { background: rgb(0, 255, 136); }
.mr-modal-close.grounded { border-color: #7d8590; color: #7d8590; }
.mr-modal-close.grounded:hover { background: #7d8590; }
...
@media (max-width: 768px) {
  ...
  .mr-mobile-close-btn { width: 100%; padding: 16px; background: transparent; border: 2px solid #ff5555; color: #ff5555; font-family: 'JetBrains Mono', monospace; font-size: 14px; font-weight: 700; letter-spacing: 2px; border-radius: 8px; cursor: pointer; }
}

/* Gallery */
.mr-gallery-modal { max-width: 100%; }
.mr-gallery-section { overflow: visible; }
...
.mr-gallery-item { min-width: 500px; height: 320px; background: rgba(17, 17, 17, 0.5); border: 1px solid rgba(255, 85, 85, 0.3); border-radius: 12px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.4s ease; flex-shrink: 0; }
.mr-gallery-item:hover { border-color: #ff5555; transform: scale(1.02); box-shadow: 0 10px 30px rgba(255, 85, 85, 0.2); }
.mr-gallery-item.expanded { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); min-width: 90vw; height: 80vh; z-index: 10003; }
...
.mr-link { display: inline-flex; align-items: center; gap: 8px; padding: 12px 24px; background: transparent; border: 1px solid #222; color: #fff; text-decoration: none; font-family: 'JetBrains Mono', monospace; font-size: 12px; font-weight: 600; border-radius: 8px; transition: all 0.3s ease; }
.mr-link:hover { border-color: rgb(0, 255, 136); color: rgb(0, 255, 136); }
.mr-link.primary { background: rgb(0, 255, 136); border-color: rgb(0, 255, 136); color: #000; }
.mr-link.primary:hover { transform: translateY(-2px); box-shadow: 0 10px 30px rgba(0, 255, 136, 0.4); }
.mr-details-links.soon .mr-link.primary { background: #58a6ff; border-color: #58a6ff; }
.mr-details-links.grounded .mr-link.primary { background: #7d8590; border-color: #7d8590; }
</style>

<style>
/* Blinking colon for timer */
.mr-blink { animation: mr-colon-blink 2s ease-in-out infinite; }
@keyframes mr-colon-blink { 0%, 100% { opacity: 1; } 50% { opacity: 0.3; } }
</style>
