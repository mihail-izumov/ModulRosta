...
// import AktivLaunches from './.vitepress/aktiv/AktivLaunches.vue'

// ─── Scroll Reveal directive (applied via ref + observer) ───
const revealEls = ref([])
let observer = null
...
onMounted(() => {
  nextTick(() => {
    counterRefs.value.forEach(({ el, end }) => {
      if (el) useCounter(el, end)
    })
  })
})
function setCounterRef(el, end) {
  if (el) counterRefs.value.push({ el, end })
}

// ─── Data ───
const heroStats = [
  { val: 10, label: 'механик', color: '#00ff88' },
  { val: 6, label: 'классов активов', color: '#ff8800' },
  { val: 14, label: 'сценариев советника', color: '#58a6ff' },
  { val: 5, label: 'категорий Q&A', color: '#ff8800' },
]

const archModules = [
  { name: 'Главный модуль', desc: 'Оркестрация всех блоков калькулятора, управление потоком данных', color: '#00ff88' },
  { name: 'Конфигурация проекта', desc: 'Все параметры, раунды, команда, риски — в одном файле. Обновляется за минуты', color: '#ff8800' },
  { name: 'Алгоритмы портфеля', desc: 'Средневзвешенная доходность, риск с поправкой на концентрацию, валидация', color: '#58a6ff' },
  { name: 'Генератор PDF', desc: 'Персональный отчёт: паспорт, портфель, дивиденды, аналитика, Q&A', color: '#ff8800' },
...
const mechanics = [
  ...
  { num: 3, title: 'Визуальная доминанта', problem: 'В списке из 6 активов основной может потеряться', solution: 'Бейдж РЕКОМЕНДУЕМ, отдельный цвет, первая позиция, блок деталей с акциями/дивидендами/покрытием.', result: 'Взгляд всегда возвращается к основному активу' },
  { num: 4, title: 'Советник', problem: 'Пользователь не понимает хорош ли портфель', solution: '14 сценариев автоматических комментариев. Проверяет условия в порядке приоритета.', result: 'При любом выборе — обратная связь с мягким nudge к увеличению основного актива' },
  { num: 5, title: 'Цель 20%+', problem: 'Нет ориентира сколько должен зарабатывать портфель', solution: 'Явный индикатор достижения 20%+ годовых. Порог выше депозитов и облигаций.', result: 'Достижим только с основным активом в портфеле' },
  { num: 6, title: 'Сценарии загрузки', problem: '«А что если не сработает?»', solution: 'Интерактивный слайдер 50-95%. Три сценария: план, умеренный, худший.', result: 'Даже при 50% — гарантированный выкуп, возврат ~190% за 4.5 года' },
  { num: 7, title: 'Покрытие залогом', problem: '«А если всё пропадёт?»', solution: 'Динамический расчёт: стоимость залога / инвестиция. Четыре уровня покрытия.', result: '1 млн — 104×, 5 млн — 21×, 10 млн — 10×' },
  { num: 8, title: 'Таймлайн', problem: '55 месяцев — абстракция', solution: '4 точки: покупка — запуск — окупаемость — выкуп. Конкретные даты.', result: 'Конкретный путь с понятными milestone\'ами' },
  ...
  { num: 10, title: 'PDF-отчёт', problem: 'Ушёл «подумать» и забыл', solution: 'Персональный документ: паспорт, портфель, дивиденды, аналитика, Q&A. Номер + дата.', result: 'Якорь, который возвращает к решению. Можно переслать партнёру.' },
]
...
const userSteps = [
  ...
  { n: '05', label: 'Детали основного актива', desc: 'Акции, дивиденды, покрытие, сценарии загрузки, таймлайн, гарантии.', color: '#00ff88' },
  { n: '06', label: 'Q&A', desc: '5 категорий × 3 вопроса. Реальные цифры. Accordion.', color: '#58a6ff' },
  { n: '07', label: 'Итог + PDF', desc: 'Портфель + доход. Кнопки: «Инвестировать» + «Скачать PDF».', color: '#ff8800' },
]
...
const poc = [
  { v: '97%', l: 'Заполняемость' }, { v: '4.7 млн', l: 'Выручка/мес' },
  { v: '2.3 млн', l: 'Прибыль/мес' }, { v: '1 495 кв.м', l: 'Площадь' },
]

const anik = [
  { v: '15 лет', l: 'На рынке' }, { v: '56', l: 'Объектов' },
  { v: '7', l: 'Городов' }, { v: '140 млн ₽', l: 'Выручка 2024' },
]

const scaleItems = ['Коммерческая недвижимость', 'Жилая недвижимость', 'Инвестфонды', 'Краудлендинг', 'Платформы для инвесторов']

const advisorTypeColor = (type) => ({
  success: '#00ff88', warning: '#ff8800', info: '#58a6ff', error: '#E8192C', neutral: 'rgba(255,255,255,0.3)'
}[type] || 'rgba(255,255,255,0.3)')

const riskWidth = (risk) => (parseInt(risk) / 10) * 100 + '%'
</script>
...
<template>
  <div class="aktiv-landing">
    ...
    <section class="hero">
      ...
      <div class="reveal" style="--d:0.55s">
        <div class="hero-stats">
          <div v-for="s in heroStats" :key="s.label" class="stat-tile">
            <div class="mono stat-val" :style="{ color: s.color }"><span :ref="el => setCounterRef(el, s.val)">0</span></div>
    ...
    <section class="sec"><div class="container">
      <div class="reveal" style="--d:0s"><div class="mono sec-label" style="color:#58a6ff">Проблема</div></div>
      <div class="reveal" style="--d:0.1s">
        <h2 class="sec-title">PDF на 40 страниц. Закрыл.<br><span class="dim">Встреча на час. Забыл.</span></h2>
      </div>
      <div class="reveal" style="--d:0.2s">
        <div class="problem-grid">
          ...
          <div class="problem-side border-left-none"><div class="mono micro-label">На стороне проекта</div><p class="problem-text">Каждый инвестор требует отдельную встречу, отдельный расчёт, отдельное объяснение. Не масштабируется.</p></div>
        </div>
      </div>
      <div class="reveal" style="--d:0.3s">
        <div class="solution-block"><div class="mono micro-label" style="color:#00ff88">Решение</div><p class="solution-text">Интерактивный калькулятор. Инвестор сам двигает слайдеры, сам собирает портфель, сам видит как меняются цифры. Всё перед глазами. Работает 24/7. Заменяет первую встречу, презентацию, Excel-модель и менеджера на ранней стадии.</p></div>
      </div>
    </div></section>
    ...
    <section class="sec"><div class="container">
      ...
      <div class="reveal" style="--d:0.1s">
        <h2 class="sec-title">Компонентная система</h2>
        <p class="sec-sub">Модульная архитектура. Все параметры проекта — в одном месте. Логика расчётов отделена от интерфейса.</p>
      </div>
          ...
          <div class="flow-row">
            <template v-for="(step, i) in flowSteps" :key="i">
              ...
              <div class="flow-step" :style="{ background: step.color + '14', borderColor: step.color + '33' }">
                <div class="mono flow-label" :style="{ color: step.color }">{{ step.label }}</div>
    ...
    <section class="sec"><div class="container">
      ...
      <div class="reveal" style="--d:0.1s">
        <h2 class="sec-title">Анализ Радаром</h2>
        <p class="sec-sub">Фирменная технология бизнес-аналитики Модуля Роста®. Из массива 300 000+ инвесторов выделены 4 стратегии с конкретным распределением между 6 классами активов.</p>
      </div>
      <div class="grid-4">
        <div v-for="(r, i) in radarStats" :key="i" class="reveal card-top" :style="{ '--d': i*0.08+'s', borderTopColor: r.color }">
          <div class="mono" :style="{ fontSize: 'clamp(22px,3vw,34px)', fontWeight: 700, color: r.color, marginBottom: '6px' }">{{ r.val }}</div>
          <div class="mono stat-label">{{ r.label }}</div>
        </div>
      </div>
      <div class="grid-auto-300 reveal" style="--d:0.3s">
        <div v-for="(f, i) in formulas" :key="i" class="formula-card" :style="{ borderLeftColor: f.color }">
          <div class="mono micro-label" :style="{ color: f.color }">{{ f.label }}</div>
          <div class="mono formula-text">{{ f.formula }}<br><span class="formula-note">{{ f.note }}</span></div>
        </div>
      </div>
    </div></section>
    ...
    <section class="sec"><div class="container">
      ...
      <div class="reveal" style="--d:0.1s"><h2 class="sec-title">Полная картина рынка</h2><p class="sec-sub">Не только основной актив. Вся картина рынка. Инвестор собирает любой портфель.</p></div>
      <div class="reveal asset-scroll" style="--d:0.2s">
        <div class="asset-table">
          ...
          <div v-for="(a, i) in assets" :key="i" class="asset-row" :class="{ highlighted: a.hl }" :style="{ borderLeftColor: a.hl ? a.color : 'transparent' }">
            ...
            <span class="mono asset-yield" :style="{ color: a.hl ? '#00ff88' : 'rgba(255,255,255,0.5)' }">{{ a.yld }}</span>
            <span class="mono asset-risk">{{ a.risk }}</span>
    ...
    <section class="sec"><div class="container">
      ...
      <div class="reveal" style="--d:0.1s"><h2 class="sec-title">Точка входа для любого инвестора</h2><p class="sec-sub">Готовые распределения с конкретными долями. Любой профиль — своя стратегия.</p></div>
      <div class="grid-4">
    ...
    <section class="sec"><div class="container">
      ...
      <div class="reveal" style="--d:0.1s"><h2 class="sec-title">14 сценариев реакции</h2><p class="sec-sub">Автоматические комментарии при каждом изменении портфеля.</p></div>
      <div class="advisor-table">
        ...
        <div v-for="(r, i) in advisorRows" :key="i" class="reveal advisor-row" :style="{ '--d': i*0.02+'s' }">
          <div class="advisor-cond">{{ r.c }}</div>
          <div class="advisor-text">{{ r.t }}</div>
          <span class="mono advisor-type" :style="{ color: advisorTypeColor(r.type) }">{{ r.type }}</span>
        </div>
      </div>
    </div></section>
      ...
      <div class="mechanics-grid">
        <div v-for="m in mechanics" :key="m.num" class="reveal mechanic" :style="{ '--d': ((m.num-1)%2)*0.05+'s' }">
          ...
          <div v-for="(b, bi) in [{ l: 'Проблема', c: '#58a6ff', t: m.problem, tc: 'rgba(255,255,255,0.45)' }, { l: 'Решение', c: '#00ff88', t: m.solution, tc: 'rgba(255,255,255,0.7)' }, { l: 'Результат', c: '#ff8800', t: m.result, tc: '#ff8800' }]" :key="bi" :style="{ marginBottom: bi < 2 ? '10px' : 0 }">
            <span class="mono micro-label" :style="{ color: b.c }">{{ b.l }}</span>
            <p class="mech-text" :style="{ color: b.tc, fontWeight: bi === 2 ? 500 : 400 }">{{ b.t }}</p>
          </div>
        </div>
      </div>
    </div></section>
    ...
    <section class="sec"><div class="container">
      ...
      <div class="grid-auto-280">
        <div v-for="(s, i) in scenarios" :key="i" class="reveal scenario-card" :style="{ '--d': i*0.08+'s', borderTopColor: s.color }">
          <div class="mono scenario-label" :style="{ color: s.color }">{{ s.label }}</div>
          <div class="scenario-roi"><span class="mono" :style="{ color: s.color }">{{ s.roi }}</span><span class="roi-suffix">годовых</span></div>
          <div class="scenario-bar"><span class="mono bar-sm">Загрузка</span><div class="bar-track"><div :style="{ width: s.load + '%', background: s.color }" /></div><span class="mono bar-val" :style="{ color: s.color }">{{ s.load }}%</span></div>
          <div class="mono scenario-detail">Прибыль: <span>{{ s.profit }}</span></div>
          <div class="mono scenario-detail">Выкуп: <span>{{ s.buyback }}</span></div>
        </div>
      </div>
        ...
        <div class="timeline">
          ...
          <div class="timeline-row">
            <div v-for="(d, i) in timeline" :key="i" class="tl-item">
              ...
              <div class="tl-dot" :class="{ active: d.active }" />
              <div class="mono tl-date" :class="{ active: d.active }">{{ d.date }}</div>
              <div class="tl-tip">{{ d.tip }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="sec"><div class="container">
      ...
      <div class="case-metrics">
        <div v-for="(m, i) in caseMetrics" :key="i" class="reveal case-metric" :style="{ '--d': i*0.03+'s' }">
          <div class="mono case-val">{{ m.val }}<span class="case-unit">{{ m.u }}</span></div>
          <div class="mono case-label">{{ m.l }}</div>
        </div>
      </div>
      <div class="poc-grid">
        <div class="reveal" style="--d:0.3s">
          <div class="poc-card green">
            <div class="mono micro-label" style="color:#00ff88">Proof of Concept · Profit Rooms · с 2021</div>
            <div class="poc-inner">
              <div v-for="(p, i) in poc" :key="i"><div class="mono poc-val green-text">{{ p.v }}</div><div class="poc-label">{{ p.l }}</div></div>
            </div>
          </div>
        </div>
        <div class="reveal" style="--d:0.35s">
          <div class="poc-card orange">
            <div class="mono micro-label" style="color:#ff8800">Партнёр · ГК «Аник»</div>
            <div class="poc-inner">
              <div v-for="(p, i) in anik" :key="i"><div class="mono poc-val orange-text">{{ p.v }}</div><div class="poc-label">{{ p.l }}</div></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="sec"><div class="container">
      ...
      <div class="grid-auto-200">
        <div v-for="(item, i) in scaleItems" :key="i" class="reveal scale-item" :style="{ '--d': i*0.04+'s' }">{{ item }}</div>
      </div>
    </div></section>
    ...
    <section class="sec" style="padding:80px 0 48px"><div class="container" style="text-align:center">
      ...
      <div class="reveal cta-buttons" style="--d:0.2s">
        <a href="/book-my-launch" class="btn-primary">Заявка</a>
        <a href="https://runscale.ru/launches" target="_blank" rel="noopener noreferrer" class="btn-ghost">Все модули</a>
      </div>
    </div></section>
    ...
    <section class="sec" style="padding:48px 0 100px"><div class="container">
      ...
      <!-- <AktivLaunches /> -->
      <div class="reveal launches-placeholder" style="--d:0.2s">
        <div class="mono" style="font-size:11px;color:rgba(255,255,255,0.2);margin-bottom:6px">&lt;!-- .vitepress/aktiv/AktivLaunches.vue --&gt;</div>
        <div style="font-size:13px;color:rgba(255,255,255,0.35)">Компонент AktivLaunches.vue — список всех модулей включая Кейс Optima Space</div>
      </div>
    </div></section>
...
.hero-stats { display: grid; grid-template-columns: repeat(4, 1fr); gap: 2px; max-width: 600px; width: 100%; }
.stat-tile { text-align: center; padding: 20px 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); }
.stat-val { font-size: 26px; font-weight: 700; }
.stat-label { font-size: 9px; letter-spacing: 1.5px; color: rgba(255,255,255,0.3); text-transform: uppercase; margin-top: 4px; }
...
.asset-yield { font-size: 13px; }
.asset-risk { font-size: 13px; color: rgba(255,255,255,0.4); }
...
.bar-sm { font-size: 10px; color: rgba(255,255,255,0.3); }
...
.scenario-label { font-size: 10px; letter-spacing: 3px; margin-bottom: 8px; }
.scenario-roi { display: flex; align-items: baseline; gap: 6px; margin-bottom: 6px; }
.scenario-roi .mono { font-size: 40px; font-weight: 700; }
.roi-suffix { font-size: 12px; color: rgba(255,255,255,0.25); }
.scenario-bar { display: flex; align-items: center; gap: 6px; margin-bottom: 12px; }
.scenario-detail { font-size: 10px; color: rgba(255,255,255,0.3); margin-bottom: 3px; }
.scenario-detail span { color: rgba(255,255,255,0.55); }
...
.guarantee-title { font-size: 13px; font-weight: 600; color: #fff; margin-bottom: 4px; }
.guarantee-desc { font-size: 11px; color: rgba(255,255,255,0.35); line-height: 1.45; }
...
.case-val { font-size: 22px; font-weight: 700; color: #fff; }
.case-unit { font-size: 11px; color: rgba(255,255,255,0.35); margin-left: 3px; }
.case-label { font-size: 10px; color: rgba(255,255,255,0.35); margin-top: 3px; letter-spacing: 1px; text-transform: uppercase; }
...
.poc-val { font-size: 20px; font-weight: 700; }
.green-text { color: #00ff88; }
.orange-text { color: #ff8800; }
.poc-label { font-size: 11px; color: rgba(255,255,255,0.35); }
...
.btn-primary { display: inline-flex; padding: 14px 40px; background: #fff; color: #ff8800; font-size: 13px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; text-decoration: none; }
.btn-ghost { display: inline-flex; padding: 14px 32px; border: 1px solid rgba(255,136,0,0.3); background: transparent; color: #ff8800; font-size: 13px; letter-spacing: 2px; text-transform: uppercase; text-decoration: none; }

/* ─── Launches ─── */
.launches-placeholder { padding: 40px 24px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); text-align: center; margin-top: 32px; }

/* ─── MOBILE ─── */
@media (max-width: 768px) {
  .problem-grid, .poc-grid { grid-template-columns: 1fr; }
  .border-left-none { border-left: 1px solid rgba(255,255,255,0.08); border-top: none; }
