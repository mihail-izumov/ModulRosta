 ...
 *
 * batch11 #8 v4: SVG-иконки листьев поверх шаров дерева.
 *   Иконки центрированы по горизонтали, нижний край совпадает с низом шара,
 *   иконки крупнее шаров и выходят за их пределы вверх.
 */
...
/** batch11 #8 v4: SVG-иконки листьев для шаров */
...
function buildCard(tpl: HomeTemplate): TplCard {
  const rooms: PillRoom[] = tpl.rooms
    .map((r): PillRoom => {
      const rt = getRT(r.typeId); const trees: Wood[] = []
      for (const fx of r.fixtures) { const q = fx.q ?? 1; for (let i = 0; i < q; i++) trees.push(fx.wood) }
      return { type: r.typeId, name: rt.name, trees, tint: ROOM_TINTS[r.typeId] }
    })
...
function darkGlassCard(extra: Record<string, string | number> = {}) {
  return {
    background: `radial-gradient(ellipse 90% 70% at 20% 0%, rgba(255,255,255,0.07), transparent 65%), radial-gradient(ellipse 70% 50% at 100% 100%, rgba(255,255,255,0.025), transparent 70%), rgba(255,255,255,0.012)`,
...
function honorBoardStyle() {
  ...
  return {
    display: 'flex', justifyContent: 'center', alignItems: 'flex-start', gap: '32px',
    padding: '26px 16px 22px', borderRadius: '12px', marginBottom: '12px',
    background: `radial-gradient(ellipse 80% 70% at 25% 15%, rgba(${r},${g},${b},0.22), transparent 70%), radial-gradient(ellipse 60% 50% at 90% 90%, rgba(${r},${g},${b},0.10), transparent 75%), rgba(${r},${g},${b},0.04)`,
    border: `1px solid rgba(${r},${g},${b},0.18)`,
    boxShadow: `inset 0 1px 0 rgba(255,255,255,0.12), 0 4px 22px rgba(0,0,0,0.3)`,
  }
}

function templateCardStyle() {
  return darkGlassCard({ width: '100%', padding: '0', borderRadius: '12px', textAlign: 'left', cursor: 'pointer', marginBottom: '8px', fontFamily: 'inherit', color: T.text, display: 'flex', alignItems: 'stretch', overflow: 'hidden' })
}

function onboardingLinkStyle() {
  return darkGlassCard({ textDecoration: 'none', marginTop: '16px', width: '100%', padding: '14px 16px', borderRadius: '12px', cursor: 'pointer', fontFamily: 'inherit', color: T.text, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', boxSizing: 'border-box' })
}
...
function orbStyle(wood: Wood, delay: number): Record<string, string> {
  ...
  return {
    ...
    '--orb-glow': `rgba(${r},${g},${b},0.45)`, '--orb-glow-soft': `rgba(${r},${g},${b},0.18)`,
    '--orb-glow-peak': `rgba(${r},${g},${b},0.55)`, '--orb-glow-peak-soft': `rgba(${r},${g},${b},0.22)`,
  }
}
...
<template>
  <div
    :style="{
      maxWidth: '480px', margin: '0 auto', padding: '32px 16px 96px',
      fontFamily: `'Segoe UI', system-ui, sans-serif`,
      color: T.text, background: T.bg, minHeight: '100vh', boxSizing: 'border-box',
    }"
  >
    ...
    <div :style="{ textAlign: 'center', fontSize: '22px', fontWeight: 700, color: T.text, marginBottom: '10px', lineHeight: 1.25 }">
      {{ formatNum(WOOD_TOTAL) }} деревьев<br />продолжают светить<br />в&nbsp;ваших домах
    </div>

    <div :style="{ textAlign: 'center', fontSize: '13px', color: T.textSec, lineHeight: 1.55, marginBottom: '20px', maxWidth: '320px', marginLeft: 'auto', marginRight: 'auto' }">
    ...
    <!-- Блок с тремя шарами + иконки листьев поверх -->
    <div :style="honorBoardStyle()">
      ...
      >
        <!-- batch11 #8 v4: обёртка для шара + SVG-иконка поверх -->
        ...
        <div :style="{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }">
          <span :style="{ fontSize: '20px', fontWeight: 700, color: T.text, fontVariantNumeric: 'tabular-nums', lineHeight: 1 }">{{ formatNum(WOOD_COUNTS[w]) }}</span>
    ...
    <a href="https://runscale.ru/woodled/onboarding" :style="onboardingLinkStyle()">
      <div :style="{ flex: 1 }">
        <div :style="{ fontSize: '13px', fontWeight: 700, color: T.text, marginBottom: '2px', lineHeight: 1.2 }">Как рождается свет</div>
        <div :style="{ fontSize: '11px', color: T.textSec, lineHeight: 1.45 }">4 главы — от ламели до теней на стене</div>
      </div>
    ...
    <div :style="{ textAlign: 'center', fontSize: '13px', color: T.textSec, lineHeight: 1.55, marginBottom: '16px', maxWidth: '320px', marginLeft: 'auto', marginRight: 'auto' }">
      Подобрали для вас самые частые сочетания параметров светильников и их расположение в доме. Начните с выбора площади – остальное настроете под себя дальше.
    </div>
        ...
        <div :style="{ flex: 1, display: 'flex', flexDirection: 'column', gap: '12px', minWidth: 0, padding: '14px' }">
          ...
          <div :style="{ display: 'flex', flexWrap: 'wrap', gap: '6px' }">
            <div v-for="(r, ri) in c.rooms" :key="ri" :style="pillStyle(r.tint)">
              ...
              <div :style="{ display: 'flex', alignItems: 'center', lineHeight: 1, paddingLeft: '3px' }">
                <span v-for="(wt, ti) in r.trees" :key="ti" :style="treeStyle(wt, ti)" />
              </div>
            </div>
          </div>
        </div>
    ...
    <button
      :style="{
        ...
        width: '100%', padding: '16px', background: T.text, color: T.bg,
        border: `1px solid ${T.text}`, borderRadius: '12px', fontSize: '14px', fontWeight: 700,
        cursor: 'pointer', fontFamily: 'inherit',
        boxShadow: `inset 0 1px 0 rgba(255,255,255,0.4), 0 4px 14px rgba(0,0,0,0.25)`,
      }"
      @click="startEmpty"
    >
      <div class="welcome-rotor" :style="{ '--rc': T.bg }" aria-hidden="true">
        <div v-for="i in 10" :key="i" class="welcome-rotor-l" :style="{ '--rot': ((i - 1) / 10 * 360) + 'deg', animationDelay: ((i - 1) * 30) + 'ms' }" />
      </div>
...
@keyframes orbPulse {
  0%, 100% {
    box-shadow: inset 0 -2px 4px rgba(0, 0, 0, 0.22), inset 0 2px 2px rgba(255, 255, 255, 0.18), 0 0 18px var(--orb-glow, rgba(196, 164, 108, 0.35)), 0 0 36px var(--orb-glow-soft, rgba(196, 164, 108, 0.12)), 0 6px 18px rgba(0, 0, 0, 0.45);
  }
  50% {
    box-shadow: inset 0 -2px 4px rgba(0, 0, 0, 0.22), inset 0 2px 2px rgba(255, 255, 255, 0.18), 0 0 28px var(--orb-glow-peak, rgba(196, 164, 108, 0.55)), 0 0 56px var(--orb-glow-peak-soft, rgba(196, 164, 108, 0.22)), 0 6px 18px rgba(0, 0, 0, 0.45);
  }
}
</style>
...
.welcome-rotor-l {
  ...
  border-radius: 1px; background: var(--rc, #b4915a);
  transform-origin: 50% 50%;
  animation: welcomeRotorCycle 5000ms ease-in-out infinite; opacity: 0;
}
